...
class TestValidatePointsLieWithinDataset():
    ...
    def test_valid_points_on_boundary(self):
        ...
        backend._validate_points_lie_within_dataset(lats, lons, self.bounds, self.res)
...
class TestGetElevationFromPath():
    ...
    def test_read_ul_corner(self):
        lats = [90]
        lons = [-180]
        z = backend._get_elevation_from_path(lats, lons, ETOPO1_GEOTIFF_PATH, 'bilinear')
    ...
    def test_bilinear_interpolation(self):
        ...
        lons = [-179.7]
        z = backend._get_elevation_from_path(lats, lons, ETOPO1_GEOTIFF_PATH, 'bilinear')
        assert pytest.approx(z[0]) == self._interp_bilinear(0.4, 0.3, self.geotiff_z[:2, :2])
    ...
    def test_error_outside_dataset(self):
        ...
        for lat, lon in zip(lats, lons):
            with pytest.raises(backend.InputError):
                z = backend._get_elevation_from_path([lat], [lon], ETOPO1_GEOTIFF_PATH, interpolation='lanczos')
...
class TestGetElevation():
    def test_single_file_dataset(self):
        ...
        elevations_by_dataset = backend.get_elevation(lats, lons, dataset, interpolation)
        elevations_by_path = backend._get_elevation_from_path(lats, lons, ETOPO1_GEOTIFF_PATH, interpolation)
